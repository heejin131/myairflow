...
try:
    ...
    if mode == "create":
        ...
        updated_meta = spark.sql("""
            SELECT 
                movieCd,
                MAX(multiMovieYn) AS multiMovieYn,
                MAX(repNationCd) AS repNationCd
            FROM (
                SELECT 
        ...
        spark.sql("""
        SELECT ASSERT_TRUE(
            (SELECT COUNT(DISTINCT movieCd) FROM temp_meta_update) == (SELECT COUNT(*) FROM temp_meta_update)
        ) AS is_unique
