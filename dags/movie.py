...
with DAG(
    ...
    # fn_merge_data(ds_nodash) 함수 완성하기 -> $HOME/data/movies/merge/dailyboxoffice/dt=20240101 저장
    def fn_merge_data(ds_nodash, base_path):
        import pandas as pd
        from movie.api.call import merge_save
        ds_nodash = '20240101'
        save_base = 'base_path'
    ...
    merge_data = PythonVirtualenvOperator(
        ...
        op_kwargs={
                "base_path": "/home/gmlwls5168/data/movies/merge/dailyboxoffice"
        }
    )
    ...
    def common_get_data(ds_nodash, url_param, base_path):
        ...
        # TODO
        # API 통해 불러온 데이터를
        # BASE_DIR/dt=20240101/repNationCd=K/****.parquet를 만들려고 함
        # STEP 1 - GIT 에서 PIP를 설치하고
        # BASE_DIR/dt=20240101/ 먼저 해보고 잘 되면!
    ...
    make_done = BashOperator(
        task_id='make.done',
        bash_command="""
        DONE_BASE=/home/gmlwls5168/data/movies/done/dailyboxoffice
